...
export default function DashboardHome() {
  ...
  const allPlans = useMemo(
    () => [...sessionPlans, ...plans].reduce((uniquePlans, plan) => {
    ...
    <Shell active="Dashboard">
      ...
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button href="/market-insights?artistId=twice" variant="outline">Start New Tour Plan</Button>
        </div>
      </div>
      <div className="mt-8 grid gap-4 md:grid-cols-4">
        <MetricCard label="Saved artists" value={allArtists.length} detail="TWICE + session artists" />
        <MetricCard label="Tour plans" value={allPlans.length} detail="TWICE demo routes" />
